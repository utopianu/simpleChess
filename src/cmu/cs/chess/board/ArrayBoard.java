...
import cmu.cs.chess.util.Iteratorable;
import cmu.cs.chess.util.Predicate;

import static cmu.cs.chess.board.ArrayPiece.*;

/**
 * 
 *0x88 Board Representation
 * 
 *The coordinate of each square on the board is represented as two
 *4-bit words. The upper 4 bits are the row, and the lower 4 bits are
 *the col.
 * 
 */
public class ArrayBoard implements Board<ArrayMove, ArrayBoard> {
	...
	public static final int[] KNIGHT_DELTAS = { UP * 2 + RIGHT, RIGHT * 2 + UP, RIGHT * 2 + DOWN, DOWN * 2 + RIGHT,
			DOWN * 2 + LEFT, LEFT * 2 + DOWN, LEFT * 2 + UP, UP * 2 + LEFT };
	public static final int[] ROOK_DELTAS = { UP, RIGHT, DOWN, LEFT };
	public static final int[] BISHOP_DELTAS = { UP_RIGHT, DOWN_RIGHT, DOWN_LEFT, UP_LEFT };
	public static final int[] QUEEN_DELTAS = merge(BISHOP_DELTAS, ROOK_DELTAS);
	public static final int[] KING_DELTAS = merge(BISHOP_DELTAS, ROOK_DELTAS);
	...
	public static final int[] FIXED_DELTAS = merge(KING_DELTAS, KNIGHT_DELTAS);
	...
	// The 0x88 representation of every valid square on the board (for
	// convenience)
	...
	public boolean equals(Object o) {
		...
		if (this.signature != b.signature)
			return false;
		if (this.colorToPlay != b.colorToPlay)
			return false;
		if (this.enpassantSquare != b.enpassantSquare)
			return false;
		if (!Arrays.equals(this.canCastleKingside, b.canCastleKingside))
			return false;
		if (!Arrays.equals(this.canCastleQueenside, b.canCastleQueenside))
			return false;
		if (!Arrays.equals(this.hasCastled, b.hasCastled))
			return false;
	...
	public void applyMove(ArrayMove move) {
		...
		// castling bits
		if (d == A1)
			setCastleQueenside(WHITE, false);
		else if (d == H1)
			setCastleKingside(WHITE, false);
		else if (d == A8)
			setCastleQueenside(BLACK, false);
		else if (d == H8)
			setCastleKingside(BLACK, false);
	 ...
	 *            the piece to move.
	 * @param dests
	 *            a list of places the source can move to.
	 * @param accumulator
	 *            the list to accumulate moves in.
	 */
	...
	private void makeCastlingMoves(ArrayPiece source, int color, List<ArrayMove> accumulator) {
		...
		if (color == WHITE) {
			if (canCastleKingside[WHITE] && source.square == E1 && board[F1].isEmpty() && board[G1].isEmpty()
					&& board[H1].piece == WHITE_ROOK) {
				ArrayMove m = ArrayMove.FACTORY.create().init(source, board[G1], board[G1]);
				accumulator.add(m);
			}

			if (canCastleQueenside[WHITE] && source.square == E1 && board[D1].isEmpty() && board[C1].isEmpty()
					&& board[B1].isEmpty() && board[A1].piece == WHITE_ROOK) {
			...
			if (canCastleKingside[BLACK] && source.square == E8 && board[F8].isEmpty() && board[G8].isEmpty()
					&& board[H8].piece == BLACK_ROOK) {
				ArrayMove m = ArrayMove.FACTORY.create().init(source, board[G8], board[G8]);
				accumulator.add(m);
			}

			if (canCastleQueenside[BLACK] && source.square == E8 && board[D8].isEmpty() && board[C8].isEmpty()
					&& board[B8].isEmpty() && board[A8].piece == BLACK_ROOK) {
	 ...
	 * 
	 * @param square
	 *            the location of the pawn making the enpassant
	 * @param accumulator
	 *            the list to accumulate pieces in.
	 */
	 ...
	 * 
	 * The destinations list will have all of the promotion moves removed from
	 * it.
	 * 
	 * @param source
	 *            the pawn to move.
	 * @param dests
	 *            all of the locations where this pawn can move.
	 * @param accumulator
	 *            the list to accumulate moves in.
	 */
	...
	public boolean isLegalMove(ArrayMove move) {
		 ...
		 * 
		 * - the src and dest squares must be on the board - the src, dest, and
		 * (promote) pieces must be chess pieces - the src piece must be at the
		 * src square - if any, the dest piece must be at the dest square - if
		 * non-enpassant capture, the capture piece must be at the dest square -
		 * if enpassant capture, the capture piece must be at the enpassant
		 * square - if any, the promote piece must be at the dest square - the
		 * src piece must be able to attack the dest square (pawn pushes and
		 * enpassants are a special case) - the src piece must be of the color
		 * toPlay - for castling, the player must be able to castle and the src
		 * piece must be a king - for promoting, the dest must be the
		 * appropriate row, the src piece must be a pawn, and the promote piece
		 * must be a QRBN of the same color as the promoting pawn - for
		 * capturing, the dest piece must be of the opposite color.
		 * 
		 * (*) If all of these are true, then the move is a pseudomove so we can
		 * just call isLegalPseudoMove to take care of check and castling
		 * through check.
		 */
		...
		if (srcpiece.piece == WHITE_PAWN && rowOfSquare(srcsq) == 6) {
			...
			if (prompiece.piece != WHITE_QUEEN && prompiece.piece != WHITE_ROOK && prompiece.piece != WHITE_BISHOP
			...
			if (prompiece.piece != BLACK_QUEEN && prompiece.piece != BLACK_ROOK && prompiece.piece != BLACK_BISHOP
		...
		if (srcpiece.piece == WHITE_KING && srcsq == E1) {
			// white kingside
			if (destsq == G1) {
				return (canCastleKingside[WHITE] && board[F1].isEmpty() && board[G1].isEmpty()
						&& board[H1].piece == WHITE_ROOK && isLegalPseudoMove(move));
			}

			// white queenside
			if (destsq == C1) {
				return (canCastleQueenside[WHITE] && board[B1].isEmpty() && board[C1].isEmpty() && board[D1].isEmpty()
						&& board[A1].piece == WHITE_ROOK && isLegalPseudoMove(move));
			}
		}
		...
		if (srcpiece.piece == BLACK_KING && srcsq == E8) {
			// black kingside
			if (destsq == G8) {
				return (canCastleKingside[BLACK] && board[F8].isEmpty() && board[G8].isEmpty()
						&& board[H8].piece == BLACK_ROOK && isLegalPseudoMove(move));
			}

			// black queenside
			if (destsq == C8) {
				return (canCastleQueenside[BLACK] && board[B8].isEmpty() && board[C8].isEmpty() && board[D8].isEmpty()
						&& board[A8].piece == BLACK_ROOK && isLegalPseudoMove(move));
			}
		}
		...
		// srcsq + k*dt = destsq for some k
			...
			if (board[destsq].isOccupied() && board[destsq].color() == srcpiece.color()) {
				return false;
			}
		}
	 ...
	 * @param pred
	 *            the predicate.
	 * @return true if the specified square is attacked by a piece which
	 *         satisfies the given predicate.
	 */
	public boolean isAttacked(int square, Predicate<ArrayPiece> pred) {
		assert (onboard(square));

		return (isSlidingAttacked(square, SLIDE_DELTAS, pred) || isFixedAttacked(square, FIXED_DELTAS, pred)
				|| isEnpassantAttacked(square, pred));
	}

	/**
	 * Fills an accumulator with all of the attacking pieces (which satisfy the
	 * predicate) on a specified square.
	 * 
	 * @param square
	 *            the attacked square.
	 * @param pred
	...
	/**
	 * Fills an accumulator with all of the enpassant attacking pieces(which
	 * satisfy the predicate) on the specified square.
	 * 
	 * @param square
	 *            the attacked square.
	 * @param pred
	 ...
	 * @param pred
	 *            the predicate.
	 * @return true if there is an attack by a piece which satisfies the
	 *         predicate on the given square.
	 */
	...
	/**
	 * @param square
	 *            the square to check for attacks against.
	 * @param deltas
	 *            the directions to check.
	 * @param pred
	 *            the predicate.
	 * @return true if there is an attacking piece (which satisfies the
	 *         predicate) on any of the deltas.
	 */
	 ...
	 *            the predicate.
	 * @return true if there is an attacking piece (which satisfies the
	 *         predicate) in any of the specified directions.
	 */
	...
	/**
	 * Fills an accumulator with all of the fixed delta attacks (which satisfy
	 * the predicate) by the specified square.
	 * 
	 * @param square
	 ...
	 *            the predicate.
	 * @param accumulator
	 *            the list to fill with attacked pieces.
	 */
	private void getAllFixedAttacksBy(int square, int[] deltas, Predicate<ArrayPiece> pred,
			List<ArrayPiece> accumulator) {
		assert (onboard(square));
	...
	private void getAllSlidingAttacksBy(int square, int[] directions, Predicate<ArrayPiece> pred,
			List<ArrayPiece> accumulator) {
		assert (onboard(square));
	...
	/**
	 * Fills an accumulator with all of the sliding attacks (which satisfy the
	 * predicate) on a given square.
	 * 
	 * @param square
	 *            the defending square.
	 * @param directions
	 *            the directions to check.
	 * @param pred
	 *            the predicate.
	 * @param accumulator
	 *            the list to fill with attacking pieces.
	 */
	private void getAllSlidingAttacksOn(int square, int[] directions, Predicate<ArrayPiece> pred,
			List<ArrayPiece> accumulator) {
		assert (onboard(square));
	...
	/**
	 * Fills an accumulator with all of the fixed attacks (which satisfy the
	 * predicate) on the specified square.
	 * 
	 * @param square
	 *            the defending square.
	 * @param deltas
	 ...
	 *            the list to fill with attacking pieces.
	 */
	private void getAllFixedAttacksOn(int square, int[] deltas, Predicate<ArrayPiece> pred,
			List<ArrayPiece> accumulator) {
		assert (onboard(square));
	 ...
	 * @param accumulator
	 *            the list to fill with the destinations.
	 */
	...
	public ArrayMove createMoveFromString(String t) {
		...
		if (t.length() == 4) {
			assert (!(d == enpassantSquare && !dest.isOccupied()
					&& source.type() == PAWN)) : "Smith string `E` suffix missing";
	...
	/**
	 * Returns the Standard Algebraic Notation string for the specified move.
	 * The string returned will be in a format suitable for use in a PGN.
	 * 
	 * WARNING: This method was created this semester (Fall 08) and so it may
	 * have bugs. (ie. I don't know of a good way to unit test this method) For
	 * example, there was a bug found for King moves in Spring 09.
	 * 
		...
		if (!castle) {
			...
			if (srcpiece.type() == PAWN) {
				...
				for (ArrayMove m : possibleMoves) {
					if (!move.equals(m) && destsq == m.dest.square && srcpiece.type() == m.source.type()) {
	...
	/**
	 * @param color
	 *            the color of the pieces to iterate over.
	...
	/**
	 * @param type
	 *            the type of the pieces to iterate over
	 * @return an iterable iterator over all pieces of the specified type.
	 */
	public Iteratorable<ArrayPiece> allPiecesOfType(int type) {
		return allPiecesMatching(new int[] { makePieceCode(WHITE, type), makePieceCode(BLACK, type) });
	}

	/**
	 * @param pieces
	 *            the piece codes to iterate over.
	 ...
	 * 
	 * @param piece
	 *            the code for the color and type of piece
	 * @return the count.
	 */
	...
	/**
	 * Counts the number of equivalent pieces of the specified type. Square
	 * piece is on is ignored.
	 * 
	 * @param p
	 *            the specified piece.
	 * @return the count.
	...
	/**
	 * Counts the number of pieces of the specified type. Color is ignored.
	 * 
	 * @param type
	 *            the type.
	 * @return the count.
	...
	/**
	 * Adds a fresh piece to the board at the square indicated by the piece.
	 * 
	 * @param p
	 *            the piece.
	 */
	...
	/**
	 * Creates and adds a piece of the specified code at the specified square.
	 * 
	 * @param piece
	 *            the code of the piece.
	 * @param square
	 *            the square to put the piece.
	 */
	...
	/**
	 * Adds a fresh piece to the board at the specified square, updating the
	 * square of the specified piece.
	 * 
	 * @param p
	 *            the piece.
	 * @param square
	 *            the square to add the piece to.
	 */
	 ...
	 * 
	 * @param square
	 *            the square of the piece to remove.
	 */
	...
	/**
	 * Removes the piece at the destination and moves the piece at the source to
	 * the destination.
	 * 
	 * @param srcSquare
	 *            the source.
	...
	/**
	 * Removes the piece at the destination and moves the specified piece to the
	 * destination.
	 * 
	 * @param p
	 *            the piece to move.
	...
	/**
	 * Performs a move where the src piece captures the dest piece, removing the
	 * dest piece form the board.
	 * 
	 * @param src
	 *            the source piece.
	 * @param dest
	...
	/**
	 * Deletes and replaces the piece in the specified square, replacing it with
	 * null the specified piece.
	 * 
	 * Updates the zobrist signature for both removing the old piece and placing
	 * the new piece at the SPECIFIED SQUARE ONLY.
	 * 
	...
	/**
	 * Overwrites the piece in the specified square with the specified piece.
	 * Ignores the square of the specified piece.
	 * 
	 * Updates the zobrist signature for both removing the old piece and placing
	 * the new piece at the SPECIFIED SQUARE ONLY.
	 * 
	 ...
	 * 
	 * @param square
	 *            the square of the piece to pickup.
	 * @return the piece.
	 */
	...
	/**
	 * Updates the zobrist signature when a piece is placed or picked up from
	 * the board.
	 * 
	...
	/**
	 * Takes the String representation of a square on a chess board and converts
	 * to the 0x88 encoded index.
	 * 
	 * @param square
	 *            the square.
	 * @return the encoded index.
	 ...
	 * 
	 * For deltas which are not a cardinal direction, returns the delta
	 * unmodified.
	 * 
	 * @param delta
	 *            the delta.
	 ...
	 * Used for debugging.
	 * 
	 * @return true if every valid square on the board contains a valid piece.
	 */

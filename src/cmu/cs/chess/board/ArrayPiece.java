...
import cmu.cs.chess.util.Creatable;
import cmu.cs.chess.util.Predicate;
import static cmu.cs.chess.board.ArrayBoard.*;

/**
 * Pieces are represented by three things - color (white or black) - type
 * (king,queen,rook,knight,bishop, or pawn) - square (square occupied on the
 * board)
 * 
 */
public class ArrayPiece implements Creatable<ArrayPiece> {
	...
	public static final int[] PIECE_TYPES = { KNIGHT, BISHOP, ROOK, QUEEN, KING, PAWN };
	...
	public static final int[] ALL_PIECES = merge(WHITE_PIECES, BLACK_PIECES);

	@SuppressWarnings("unchecked")
	public static final Predicate<ArrayPiece>[] P_IS_COLOR = new Predicate[2];
	static {
		P_IS_COLOR[WHITE] = new Predicate<ArrayPiece>() {
			public boolean check(ArrayPiece t) {
				return (t.isOccupied() && t.color() == WHITE);
			}
		};
		P_IS_COLOR[BLACK] = new Predicate<ArrayPiece>() {
			public boolean check(ArrayPiece t) {
				return (t.isOccupied() && t.color() == BLACK);
			}
		};
	}
	@SuppressWarnings("unchecked")
	...
	/**
	 * @return Assuming this piece is a sliding piece, returns true iff this
	 *         piece can slide horizontally and vertically.
	 */
			...
			for (int source = 0; source <= UP_RIGHT_CORNER; ++source) {
				...
				for (int dest = 0; dest <= UP_RIGHT_CORNER; ++dest) {
					...
					if (piece.type() == QUEEN || piece.type() == ROOK) {
						if (absdelta < 8)
							matt[index] = true; // on same row
						if (absdelta % UP == 0)
							matt[index] = true; // on same col
					}

					if (piece.type() == QUEEN || piece.type() == BISHOP) {
						if (absdelta % UP_LEFT == 0)
							matt[index] = true; // up-left or down-left
						if (absdelta % UP_RIGHT == 0)
							matt[index] = true; // up-right or down-right
					}

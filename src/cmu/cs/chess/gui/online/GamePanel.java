...
import cmu.cs.chess.engine.Engine;
import cmu.cs.chess.server.Hub;
import cmu.cs.chess.server.L2Codes;
import cmu.cs.chess.server.L2Item;
...
public final class GamePanel extends JPanel implements Observer {
   ...
   */
	...
	public void receiveL2(L2Item item) {
		...
		case L2Codes.STARTED_OBSERVING:
			...
			if (!Config.boolGuest && amPlaying) {
				// m_player.newGame(180000,2000); //added by Pravir
				m_player = new Engine(hub, item.getInt(7) * 60000,
						item.getInt(8) * 1000);
	...
	private class MakeMoveRunner implements Runnable {
		...
		public void run() {
			try {
				String ur_move = m_player.computeMove(
						(int) lowerClock.msecleft, (int) upperClock.msecleft)
						.serverString();

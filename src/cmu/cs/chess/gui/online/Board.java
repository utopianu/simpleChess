package cmu.cs.chess.gui.online;
...
public final class Board extends Canvas {
	...
	/**
	 * The width of the line that will be drawn to show the current best move as
	...
	public void newGame(String initial) {
		...
		for (row = 0; row < 8; row++)
			for (col = 0; col < 8; col++)
				pieces[row][col] = initial.charAt(row * 8 + col);
	}
	...
	public void paint(Graphics g) {
		...
		if (bestMove1 != null && bestMove2 != null) {
			...
			if (g instanceof Graphics2D) {
				...
				g2d.setStroke(new BasicStroke(bestMoveWidth, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER));
		...
		for (row = 0; row < 8; row++) {
			for (col = 0; col < 8; col++) {
				painter.drawPiece(g, getPiece(col, row), squareSize, col * squareSize, row * squareSize);
			}
		}
		...
		if (isPending) {
			...
			if ((pendingFrom.x + pendingFrom.y) % 2 == 0) // even square,
				...
				g.setColor(colorDarkSquare);
			g.fillRect(pendingFrom.x * squareSize, pendingFrom.y * squareSize, squareSize, squareSize);
			...
			if (getPiece(pendingTo) != '-') {
				if ((pendingTo.x + pendingTo.y) % 2 == 0) // even square,
					...
					g.setColor(colorDarkSquare);
				g.fillRect(pendingTo.x * squareSize, pendingTo.y * squareSize, squareSize, squareSize);
			}

			// draw piece at square it was moved to
			painter.drawPiece(g, getPiece(pendingFrom), squareSize, pendingTo.x * squareSize, pendingTo.y * squareSize);
		}

		// draw highlights
		g.setColor(colorHighlight);
		if (highlit1 != null) {
			g.drawRect(highlit1.x * squareSize, highlit1.y * squareSize, squareSize - 1, squareSize - 1);
		}
		if (highlit2 != null) {
			g.drawRect(highlit2.x * squareSize, highlit2.y * squareSize, squareSize - 1, squareSize - 1);
		}
		...
		case 'k':
			...
			if (absdx == 2 && dy == 0 && bdFrom.y == 7) {
				...
				if (!isFlipped) {
					...
					if (dx == -2) {
						// long
						return (longCastleOK && getPiece(0, 7) == 'R' && clearPath(bdFrom, new Point(1, 7)));
					} else {
						// short
						return (shortCastleOK && getPiece(7, 7) == 'R' && clearPath(bdFrom, bdTo));
					}
				} else {
					// black castle
					if (bdFrom.x != 3)
					...
					if (dx == -2) {
						// short
						return (shortCastleOK && getPiece(0, 7) == 'r' && clearPath(bdFrom, bdTo));
					} else {
						// long
						return (longCastleOK && getPiece(7, 7) == 'r' && clearPath(bdFrom, new Point(6, 7)));
					}
				}
			}
			return false;
	...
	public Point boardToScreen(Point p) {
		return new Point(p.x * squareSize + (squareSize / 2), p.y * squareSize + (squareSize / 2));
	}
		...
		if (Config.boolGuest) {
			...
			if (x < 8 * squareSize && y < 8 * squareSize) {
				...
				if (algFrom.equals(algTo)) {
					...
					if ((p2.y == 0 || p2.y == 7) && Character.toUpperCase(getPiece(p1)) == 'P') {
	...
	public void setBestMove(String smithBestMove) {
		...
		// Debuggin'
		// System.err.println("New best move has not been drawn: " + bestMove +
		// "\n");
		// setHighlights(bestMove.serverString());
	}

	/*
	 * public static void main(String args[]) throws Exception { Frame f = new
	 * Frame("stub");
	 * 
	 * Board c1 = new Board(null, null);
	 * 
	 * f.setLayout(new GridLayout(1, 1) ); f.add(c1); f.pack(); f.show();
	 * c1.setFlipped(true);
	 * 
	 * String s=""; char c; while (true) { c = (char) System.in.read(); if (c ==
	 * '\n') { c1.doSmithMove(s); c1.repaint(); s = ""; } else { s += "" + c; }
	 * } }
	 */
}
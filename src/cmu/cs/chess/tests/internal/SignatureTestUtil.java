...
public class SignatureTestUtil {
	public static <M extends Move<M>, B extends Board<M, B>> int checkSigsAll(B b, int depth) {
		return checkSigs(b, depth, PerftTestUtil.database.keySet().toArray(new String[0]));
	}

	public static <M extends Move<M>, B extends Board<M, B>> int checkSigs(B b, int depth, String... fens) {
	...
	private static <M extends Move<M>, B extends Board<M, B>> int checkSig(M m, B b, int depth, List<B> seen,
			Map<Long, B> sigs) {
		if (depth == 0)
			return 0;
